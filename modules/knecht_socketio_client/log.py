...
from modules.globals import DEV_LOGGER_NAME, FROZEN, SOCKET_LOG_FILE_NAME, SOCKET_MAIN_LOGGER_NAME, get_settings_dir


def setup_logging(logging_queue, overwrite_level: str=None):
    # Track calls to this method
    print('Socket Proc Logging setup called: ',
    ...
    log_file_path = Path(get_settings_dir()) / Path(SOCKET_LOG_FILE_NAME)
...
def init_logging(logger_name):
    ...
    print('Knecht SocketIO Client Logger requested by: ',
          Path(sys._getframe().f_back.f_code.co_filename).name,
