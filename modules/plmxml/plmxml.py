...
class PlmXml:
    ...
    def _read_product_instance_element(self, n):
        ...
        instance_id = n.attrib.get(Pg.PRODUCT_INSTANCE_TAGS.get('id'))
        ...
        if name == Pg.LOOK_LIBRARY_INSTANCE_NAME:
    ...
    def _read_node_user_data(n: Et._Element):
        ...
        for user_node in n.iterfind(Pg.USER_DATA_XPATH):
    ...
    def parse_plm_xml(self):
        if not path_exists(self.file):
            self.error = _('PlmXml Datei wurde nicht gefunden.')
        ...
        for n in root.iterfind(Pg.PRODUCT_INSTANCE_XPATH):
